...
def testListOutput(serialno=None, Protocol=pmapi.USB_protocol()):
    ...
    samples[sampleEngine.channels.timeStamp]
    samples[sampleEngine.channels.MainCurrent]
    samples[sampleEngine.channels.AuxCurrent]
    samples[sampleEngine.channels.USBCurrent]
    samples[sampleEngine.channels.MainVoltage]
...
def testCSVOutput(serialno=None, Protocol=pmapi.USB_protocol()):
    ...
    Engine.startSampling(
        3000000, 100
    )  # Collect 10 minutes worth of samples a 1/100 granularity
    Mon.closeDevice()
