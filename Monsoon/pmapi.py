...
class USB_protocol:
    ...
    def __init__(self):
        # Any: filled in by Connect(); see the note in LVPM.__init__.
    ...
    def Connect(self, deviceType, serialno=None):
        ...
        def device_matcher(d):
            try:
                return (
                    ...
                    and (serialno is None or d.serial_number == str(serialno))
                )
            except Exception:  # Catches some platform-specific errors when connecting to multiple PMs simultaneously.
                return False
        ...
        if connectedDeviceType != deviceType:
            print(
                "warning:  Device type mismatch.  Found "
                + repr(connectedDeviceType)
                + " expected "
                + repr(deviceType)
            )
        ...
        if firmwareRev < op.ReturnCodes.CURRENT_FIRMWARE_REV:
            print(
                "Warning:  Detected firmware revision "
                + repr(firmwareRev)
                + ", current release is "
                + repr(op.ReturnCodes.CURRENT_FIRMWARE_REV)
            )
