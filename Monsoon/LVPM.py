...
class Monsoon:
    def __init__(self, *args, **kwargs):
        # Annotated Any because these are placeholders: setup_usb()/Connect() fill
        # them in, and every method that uses them runs only afterwards. Without the
        # annotation a type checker reads the declared type as None and rejects every
        # use, which is 100+ diagnostics saying the same thing about a pattern that is
        # correct as long as the caller connects first.
        self.DEVICE: Any = None
        self.DEVICE_TYPE: Any = None
        self.epBulkWriter: Any = None
        self.epBulkReader: Any = None
    ...
    def setup_usb(self, serialno=None, Protocol=None):
        ...
        Protocol defaulted to pmapi.USB_protocol() upstream -- evaluated once, when the class
        body ran, so every unit that did not pass one explicitly shared a single
        protocol object and therefore a single device handle. Two units is exactly
        what multiPMExample.py demonstrates.
        """
        if Protocol is None:
    ...
    # BOTH of these called self.__raw_from_amps(), which Python name-mangles inside a
    # class body to self._Monsoon__raw_from_amps -- a method that does not exist. So
    # every attempt to set a current limit on an LVPM raised AttributeError, and had
    # since the method was written; HVPM.py has the same two methods spelled
    # correctly. The practical effect is that an LVPM ran with whatever limits were in
    # its EEPROM, and a caller that thought it had protected the device under test had
    # not.
    ...
    def fillStatusPacket(self):
        ...
        # Calibration data
        self.statusPacket.mainFineResistorOffset = float(
            self.Protocol.getValue(op.OpCodes.setMainFineResistorOffset, 1)
        )
        mainFineResistor = (
            self.factoryRes + self.statusPacket.mainFineResistorOffset * 0.0001
        )
        self.statusPacket.mainCoarseResistorOffset = float(
            self.Protocol.getValue(op.OpCodes.setMainCoarseResistorOffset, 1)
        )
        mainCoarseResistor = (
            self.factoryRes + self.statusPacket.mainCoarseResistorOffset * 0.0001
        )
        self.statusPacket.usbFineResistorOffset = float(
            self.Protocol.getValue(op.OpCodes.setUsbFineResistorOffset, 1)
        )
        usbFineResistor = (
            self.factoryRes + self.statusPacket.usbFineResistorOffset * 0.0001
        )
        self.statusPacket.usbCoarseResistorOffset = float(
            self.Protocol.getValue(op.OpCodes.setUsbCoarseResistorOffset, 1)
        )
        usbCoarseResistor = (
            self.factoryRes + self.statusPacket.usbCoarseResistorOffset * 0.0001
        )
        self.statusPacket.auxFineResistorOffset = float(
            self.Protocol.getValue(op.OpCodes.setAuxFineResistorOffset, 1)
        )
        auxFineResistor = (
            self.auxFactoryResistor + self.statusPacket.auxFineResistorOffset * 0.0001
        )
        self.statusPacket.auxCoarseResistorOffset = float(
            self.Protocol.getValue(op.OpCodes.setAuxCoarseResistorOffset, 1)
        )
        auxCoarseResistor = (
            self.auxFactoryResistor + self.statusPacket.auxCoarseResistorOffset * 0.0001
        )

        # LVPM Scaling system.
        self.statusPacket.mainFineScale = 35946.0 * (self.factoryRes / mainFineResistor)
        self.statusPacket.mainCoarseScale = 3103.4 * (
            self.factoryRes / mainCoarseResistor
        )
        self.statusPacket.usbFineScale = 35946.0 * (self.factoryRes / usbFineResistor)
        self.statusPacket.usbCoarseScale = 3103.4 * (
            self.factoryRes / usbCoarseResistor
        )
        self.statusPacket.auxFineScale = 0.05049 * (
            self.auxFactoryResistor / auxFineResistor
        )
        self.statusPacket.auxCoarseScale = 1.289 * (
            self.auxFactoryResistor / auxCoarseResistor
        )
    ...
    def swizzlePacket(self, packet):
        ...
        swizzledPacket = np.hstack(
            [packet[0:4], swizzledBytes[:], self.__padding[0 : (58 - length)]]
        )
