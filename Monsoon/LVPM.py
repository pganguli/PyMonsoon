...
class Monsoon(object):
    def __init__(self, *args, **kwargs):
        ...
        self.__padding = np.zeros(64, dtype=int)
    ...
    def setUSBCoarseZeroOffset(self, value):
        ...
        self.Protocol.sendCommand(op.OpCodes.SetUSBCoarseZeroOffset, int(value))

    def setUSBPassthroughMode(self, USBPassthroughCode):
        """USB Passthrough mode. 0 = off, 1 = on, 2 = auto"""
        self.Protocol.sendCommand(
            op.OpCodes.setUsbPassthroughMode, int(USBPassthroughCode)
        )

    def setVoltageChannel(self, VoltageChannelCode):
        """Sets voltage measurement channel. 0 = Main & USB, 1 = Main & Aux"""
        self.Protocol.sendCommand(op.OpCodes.setVoltageChannel, int(VoltageChannelCode))
