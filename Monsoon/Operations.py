class OpCodes:
    ...
    setUsbPassthroughMode = (
        0x10  # Sets USB Passthrough mode according to value.  Off = 0, On = 1, Auto = 2
    )
...
class statusPacket:
    ...
    The scale and resistor-offset fields are annotated float rather than left to be
    inferred from their 0 initialiser: LVPM.fillStatusPacket and HVPM.fillStatusPacket
    both store genuine floats in them -- a resistor offset is ohms = 0.05 + 0.0001*n --
    so an int-typed field described the placeholder rather than the value."""
    ...
    powerupCurrentLimit: float = 0  # Max current during startup before overcurrent protection circuit activates.  LVPM is 0-8A, HVPM is 0-15A.
    runtimeCurrentLimit: float = 0  # Max current during runtime before overcurrent protection circuit activates.  LVPM is 0-8A, HVPM is 0-15A.
    ...
    mainFineScale: float = 0  # HVPM Calibration value, 32-bits, unsigned
    mainCoarseScale: float = 0  # HVPM Calibration value, 32-bits, unsigned
    usbFineScale: float = 0  # HVPM Calibration value, 32-bits, unsigned
    usbCoarseScale: float = 0  # HVPM Calibration value, 32-bits, unsigned
    auxFineScale: float = 0  # HVPM Calibration value, 32-bits, unsigned
    auxCoarseScale: float = 0  # HVPM Calibration value, 32-bits, unsigned

    mainFineZeroOffset: float = 0  # HVPM-only, Zero-level offset
    mainCoarseZeroOffset: float = 0  # HVPM-only, Zero-level offset
    usbFineZeroOffset: float = 0  # HVPM-only, Zero-level offset
    usbCoarseZeroOffset: float = 0  # HVPM-only, Zero-level offset
    hardwareModel = 0  # HVPM-only, Zero-level offset

    mainFineResistorOffset: float = 0  # signed, ohms = 0.05 + 0.0001*offset
    mainCoarseResistorOffset: float = 0  # signed, ohms = 0.05 + 0.0001*offset
    usbFineResistorOffset: float = 0  # signed, ohms = 0.05 + 0.0001*offset
    usbCoarseResistorOffset: float = 0  # signed, ohms = 0.05 + 0.0001*offset
    auxFineResistorOffset: float = 0  # signed, ohms = 0.10 + 0.0001*offset
    auxCoarseResistorOffset: float = 0  # signed, ohms = 0.10 + 0.0001*offset
